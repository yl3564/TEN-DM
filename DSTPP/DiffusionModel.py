...
class SinusoidalPosEmb(nn.Module):
    ...
    def forward(self, x):
        ...
        emb = torch.cat((emb.sin(), emb.cos()), dim=-1) # [Batch, self.dim]
        return emb 
...
class ST_Diffusion(nn.Module):
    def __init__(self, n_steps, dim, num_units=64,self_condition = False,condition=True,cond_dim=0):
        ...
        self.linear_t_2 = nn.Sequential(
                nn.Linear(num_units, num_units),
                nn.ReLU(),
                nn.Linear(num_units, 2)
        )

        self.linear_s_2 = nn.Sequential(
                nn.Linear(num_units, num_units),
                nn.ReLU(),
                nn.Linear(num_units, 2)
        )
        ...
        self.cond_t = nn.Sequential(
                nn.Linear(cond_dim * 2, num_units),
                nn.ReLU(),
                nn.Linear(num_units, num_units)
        )
        ...
        self.graph_emb_all = nn.Sequential(
                nn.Linear(2, num_units),
                nn.ReLU(),
                nn.Linear(num_units, num_units)
        )

        self.cond_graph_emb = nn.ModuleList(
            [
                nn.Linear(2, num_units),
                nn.Linear(2, num_units),
                nn.Linear(2, num_units)
    ...
    def get_attn(self, x, t, x_self_cond = None, cond = None):
        ...
        cond_temporal, cond_spatial, cond_joint, graph_emb = cond[:,:,:hidden_dim], cond[:,:,hidden_dim:2*hidden_dim], cond[:,:,2*hidden_dim:3*hidden_dim], cond[:,:,3*hidden_dim:(3*hidden_dim+2)]

        cond = self.cond_all(cond[:,:,:3*hidden_dim])
        t_embedding = self.time_mlp(t).unsqueeze(dim=1)

        graph_emb_all = self.graph_emb_all(graph_emb)
        cond_all = torch.cat((cond, t_embedding, graph_emb_all),dim=-1)
        alpha_s = F.softmax(self.linear_s_1(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2) # [N, 2, 1]
        alpha_t = F.softmax(self.linear_t_1(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2) # [N, 2, 1]
    ...
    def forward(self, x, t, x_self_cond = None, cond = None):
        ...
        hidden_dim = 64
        cond_temporal, cond_spatial, cond_joint, graph_emb = cond[:,:,:hidden_dim], cond[:,:,hidden_dim:2*hidden_dim], cond[:,:,2*hidden_dim:3*hidden_dim], cond[:,:,3*hidden_dim:(3*hidden_dim+2)]
        cond = self.cond_all(cond[:,:,:3*hidden_dim]) # [N, 1, 64]
        t_embedding = self.time_mlp(t).unsqueeze(dim=1) # [N, 1, 64]

        graph_emb_all = self.graph_emb_all(graph_emb) # [N, 1, 64]
        cond_all = torch.cat((cond, t_embedding, graph_emb_all),dim=-1)
        alpha_s = F.softmax(self.linear_s_1(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2) # [N, 2, 1]
        alpha_t = F.softmax(self.linear_t_1(cond_all), dim=-1).squeeze(dim=1).unsqueeze(dim=2) # [N, 2, 1]
        ...
        for idx in range(3):
            ...
            cond_graph_emb = self.cond_graph_emb[idx](graph_emb)
            x_spatial += cond_joint_emb + cond_spatial_emb + cond_graph_emb
            x_temporal += cond_joint_emb + cond_temporal_emb + cond_graph_emb
